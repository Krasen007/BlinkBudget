 ...
 */
export const prepareChartData = (transactions, period = 6, calcMethod = 'average', _chartType = 'line', categories = null, referenceDate = new Date()) => {
  let topCategories;
  ...
  if (categories && categories.length > 0) {
    ...
    topCategories = categories.map(catName => {
      const inflationRate = PersonalInflationService.calculateCategoryInflation(transactions, catName, period, calcMethod, referenceDate);
      return {
        category: catName,
        inflationRate,
      ...
      tooltip: {
        callbacks: {
          label: function(context) {
            ...
            // Extract inflation rate from label
            const inflationMatch = label.match(/\(([-\d.]+)% personal inflation\)/);
            const inflationText = inflationMatch ? inflationMatch[1] : '';
            
            return `${label.replace(inflationText, '').trim()}: ${formattedValue} ${inflationText}`;
          }
        }
      }
    },
