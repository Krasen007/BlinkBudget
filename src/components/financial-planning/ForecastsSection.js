 ...
 */
export const ForecastsSection = ({ planningData, chartRenderer, onChartCreate }) => {
  const section = SectionContainer({
    id: 'forecasts',
    title: 'Financial Forecasts',
    icon: '🔮',
    usageNote: 'Forecasts use your past 3+ months of transactions to predict income and expenses. Adjust assumptions in the scenario tab to see how changes affect future balances.'
  });

  if (!planningData || !planningData.transactions || planningData.transactions.length < 3) {
    const placeholder = Placeholder({
      title: 'Insufficient Data for Forecasting',
      description: 'Add at least 3 months of transaction history to generate accurate financial forecasts.',
      icon: '📊'
    });
  ...
  try {
    ...
    const errorPlaceholder = Placeholder({
      ...
      description: 'There was an error processing your transaction data. Please try refreshing the page.',
      icon: '⚠️'
    });
...
function createForecastTable(incomeForecasts, expenseForecasts) {
  ...
  for (let i = 0; i < maxRows; i++) {
    ...
    const row = [
      income.period ? income.period.toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) : `Month ${i + 1}`,
    ...
    const rowStyles = [
      ...
      { color: net >= 0 ? COLORS.SUCCESS : COLORS.ERROR, fontWeight: '600', textAlign: 'right' }, // Net
      {
