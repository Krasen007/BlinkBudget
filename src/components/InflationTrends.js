...
import { prepareChartData, getChartOptions, validateChartData } from '../utils/inflation-chart-utils.js';
import { InsightsGenerator } from '../core/insights-generator.js';

export const InflationTrends = (data, chartRenderer, activeCharts, sharedMonthState = null) => {
  ...
  canvas.id = 'inflation-trends-chart';
  ...
  const getSelectedMonthTransactions = () => {
    ...
    const now = new Date();
    const targetDate = new Date(now.getFullYear(), now.getMonth() + sharedMonthState.offset, 1);
  ...
  const renderChart = async () => {
    try {
      ...
      const referenceDate = sharedMonthState 
        ? new Date(now.getFullYear(), now.getMonth() + sharedMonthState.offset + 1, 0)
        : now;
      ...
      // Create chart
      currentChart = await chartRenderer[`create${currentChartType === 'line' ? 'Line' : 'Bar'}Chart`](
        canvas,
        { labels, datasets },
    ...
    container.innerHTML = `
      <div style="display: flex; align-items: center; justify-content: center; height: 100%; color: var(--text-secondary);">
        <div style="text-align: center;">
          ...
          <div>Not enough data for inflation analysis</div>
          <div style="font-size: 0.875rem; margin-top: 0.25rem;">Need at least 2 months of spending data</div>
        </div>
      </div>
    `;
  };
