...
export const BudgetSummaryCard = (summary, timePeriod = null) => {
  ...
  const progressContainer = BudgetProgress({
    ...
    isExceeded:
      summary.totalActual > summary.totalLimit && summary.totalLimit > 0,
    isWarning:
      summary.overallUtilization >= 80 && summary.overallUtilization <= 100,
  ...
  if (summary.totalOverspent > 0) {
    details.appendChild(
      createDetail(
        'Overspent',
        formatCurrency(summary.totalOverspent),
        COLORS.ERROR
      )
    );
  }
