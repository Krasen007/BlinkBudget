...
export class ProgressiveDataLoader {
  ...
  async processDataInChunks(transactions, timePeriod) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      await this.processChunk(chunk, accumulators);
  ...
  sortTransactionsForProcessing(transactions, prioritizeCategories) {
    ...
    return transactions.sort((a, b) => {
      const aCategoryPriority = PROGRESSIVE_CONFIG.PRIORITY_CATEGORIES.indexOf(
        a.category || ''
      );
      const bCategoryPriority = PROGRESSIVE_CONFIG.PRIORITY_CATEGORIES.indexOf(
        b.category || ''
      );
