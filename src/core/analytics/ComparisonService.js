...
export class ComparisonService {
  ...
  static comparePeriodsSpending(transactions, currentPeriod, comparisonPeriod) {
    const currentData = {
      incomeVsExpenses: MetricsService.calculateIncomeVsExpenses(
        transactions,
        currentPeriod
      ),
      categoryBreakdown: MetricsService.calculateCategoryBreakdown(
        transactions,
        currentPeriod
      ),
    ...
    const behaviorChanges = this.analyzeBehaviorChanges(
      transactions,
      currentPeriod,
      comparisonPeriod
    );
  ...
  static calculateOverallComparison(currentData, comparisonData) {
    ...
    const balanceChangePercent =
      comparison.netBalance !== 0
        ? (balanceChange / Math.abs(comparison.netBalance)) * 100
        : 0;

    const transactionCountChange =
      current.expenseCount - comparison.expenseCount;
    const transactionCountChangePercent =
      comparison.expenseCount > 0
        ? (transactionCountChange / comparison.expenseCount) * 100
        : 0;

    const avgTransactionChange =
      current.averageExpense - comparison.averageExpense;
    const avgTransactionChangePercent =
      comparison.averageExpense > 0
  ...
  static calculateCategoryComparison(currentCategories, comparisonCategories) {
    ...
    for (const categoryName of allCategories) {
      ...
      if (currentCat && comparisonCat) {
        ...
        const amountChangePercent =
          comparisonCat.amount > 0
            ? (amountChange / comparisonCat.amount) * 100
            : 0;
        const percentagePointChange =
          currentCat.percentage - comparisonCat.percentage;
        ...
        categoryChanges.push({
          ...
          changes: {
            ...
            transactionCount:
              currentCat.transactionCount - comparisonCat.transactionCount,
          },
          trend:
            amountChange > 0
              ? 'increased'
              : amountChange < 0
                ...
                : 'stable',
          significance:
            Math.abs(amountChangePercent) > 25
              ? 'high'
              : Math.abs(amountChangePercent) > 10
                ? 'medium'
                : 'low',
        });
    ...
    categoryChanges.sort((a, b) => {
      ...
      return (
        Math.abs(b.changes.amountPercent) - Math.abs(a.changes.amountPercent)
      );
    });
    ...
    return {
      ...
      significantChanges: categoryChanges.filter(c => c.significance === 'high')
        .length,
    };
  }
  ...
  static analyzeBehaviorChanges(transactions, currentPeriod, comparisonPeriod) {
    const currentTransactions = FilteringService.filterByTimePeriod(
      transactions,
      currentPeriod
    ).filter(t => t.type === TRANSACTION_TYPES.EXPENSE);
    const comparisonTransactions = FilteringService.filterByTimePeriod(
      transactions,
      comparisonPeriod
    ).filter(t => t.type === TRANSACTION_TYPES.EXPENSE);

    const frequencyChanges = this.analyzeFrequencyChanges(
      currentTransactions,
      comparisonTransactions,
      currentPeriod,
      comparisonPeriod
    );
    const timingChanges = this.analyzeTimingChanges(
      currentTransactions,
      comparisonTransactions
    );
    const sizeChanges = this.analyzeSizeChanges(
      currentTransactions,
      comparisonTransactions
    );
    ...
    return {
      ...
      timingChanges,
      sizeChanges,
      summary: this.generateBehaviorChangeSummary(
        frequencyChanges,
        timingChanges,
        sizeChanges
      ),
  ...
  static analyzeFrequencyChanges(
    currentTransactions,
    comparisonTransactions,
    currentPeriod,
    comparisonPeriod
    ...
    const currentDays =
      Math.ceil(
        (new Date(currentPeriod.endDate) - new Date(currentPeriod.startDate)) /
          (1000 * 60 * 60 * 24)
      ) + 1;
    const comparisonDays =
      Math.ceil(
        (new Date(comparisonPeriod.endDate) -
          new Date(comparisonPeriod.startDate)) /
          (1000 * 60 * 60 * 24)
      ) + 1;
      ...
      trend:
        frequencyChange > 0
          ...
          : frequencyChange < 0
            ...
            : 'stable',
      significance:
        Math.abs(frequencyChangePercent) > 25
          ? 'high'
          : Math.abs(frequencyChangePercent) > 10
            ? 'medium'
            : 'low',
    };
  }
  ...
  static analyzeTimingChanges(currentTransactions, comparisonTransactions) {
    ...
    const currentDayPattern =
      this.calculateDayOfWeekPattern(currentTransactions);
    const comparisonDayPattern = this.calculateDayOfWeekPattern(
      comparisonTransactions
    );
    ...
    const significantDayChanges = Object.entries(dayOfWeekChanges)
      .filter(([_day, data]) => Math.abs(data.change) > 10)
  ...
  static analyzeSizeChanges(currentTransactions, comparisonTransactions) {
    const currentAmounts = currentTransactions.map(t =>
      Math.abs(t.amount || 0)
    );
    const comparisonAmounts = comparisonTransactions.map(t =>
      Math.abs(t.amount || 0)
    );
    ...
    return {
      ...
      significance:
        Math.abs(averageChangePercent) > 20
          ? 'high'
          : Math.abs(averageChangePercent) > 10
            ? 'medium'
            : 'low',
    };
  }
  ...
  static calculateTransactionSizeStats(amounts) {
    ...
    const median =
      amounts.length % 2 === 0
        ? (sortedAmounts[amounts.length / 2 - 1] +
            sortedAmounts[amounts.length / 2]) /
          2
  ...
  static generateBehaviorChangeSummary(
    frequencyChanges,
    timingChanges,
    sizeChanges
  ) {
  ...
  static generateComparisonInsights(
    ...
    if (Math.abs(overallComparison.netBalance.changePercent) > 15) {
      insights.push({
        ...
        type:
          overallComparison.netBalance.trend === 'improved'
            ? 'positive'
            : 'warning',
        ...
        severity:
          Math.abs(overallComparison.netBalance.changePercent) > 30
            ? 'high'
            : 'medium',
    ...
    if (Math.abs(overallComparison.expenses.changePercent) > 10) {
      insights.push({
        id: 'expense_trend',
        type:
          overallComparison.expenses.trend === 'increased'
            ? 'warning'
            : 'positive',
        ...
        severity:
          overallComparison.expenses.trend === 'increased' &&
          overallComparison.expenses.changePercent > 25
            ? 'high'
            : 'medium',
        actionable: overallComparison.expenses.trend === 'increased',
      });
    }
    categoryComparison.categoryChanges
      .filter(c => c.significance === 'high')
      .slice(0, 3)
      .forEach(categoryChange => {
        insights.push({
      ...
      behaviorChanges.summary.changes.forEach(change => {
        insights.push({
          ...
          actionable:
            ...
            change.description.includes('increased'),
        });
      });
    }
    return insights;
  ...
  static generateComparisonSummary(overallComparison, categoryComparison) {
    ...
    if (Math.abs(overallComparison.expenses.changePercent) > 10)
      summary.keyChanges.push(
        `Expenses ${overallComparison.expenses.trend} by ${Math.abs(overallComparison.expenses.changePercent).toFixed(1)}%`
      );
    if (Math.abs(overallComparison.income.changePercent) > 10)
      summary.keyChanges.push(
        `Income ${overallComparison.income.trend} by ${Math.abs(overallComparison.income.changePercent).toFixed(1)}%`
      );
    const topCategoryChange = categoryComparison.categoryChanges[0];
    if (topCategoryChange && topCategoryChange.significance === 'high')
      summary.keyChanges.push(
        `${topCategoryChange.category} spending ${topCategoryChange.trend} by ${Math.abs(topCategoryChange.changes.amountPercent).toFixed(1)}%`
      );

    if (
      overallComparison.expenses.trend === 'increased' &&
      overallComparison.expenses.changePercent > 20
    )
      summary.recommendations.push(
        'Consider reviewing your spending patterns to identify areas for reduction'
      );
    if (categoryComparison.significantChanges > 0)
      summary.recommendations.push(
        'Focus on categories with the largest spending increases'
      );
  ...
  static getHistoricalInsights(transactions, historicalPeriods) {
    const historicalInsights = historicalPeriods.map(period => {
      const periodInsights = InsightsService.generateSpendingInsights(
        transactions,
        period
      );
      return {
        ...
        summary: {
          ...
          highSeverityInsights: periodInsights.filter(
            i => i.severity === 'high'
          ).length,
  ...
  static calculateOverallTrendDirection(trends) {
    ...
    return {
      ...
      severity: calculateSimpleTrend(
        trends.severityTrends.map(t => t.highSeverity)
      ),
      actionable: calculateSimpleTrend(
        trends.actionableInsightTrends.map(t => t.actionable)
      ),
    };
  }
}
