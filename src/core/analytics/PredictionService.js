...
export class PredictionService {
  ...
  static predictFutureSpending(
    ...
    if (!historicalAnalysis.hasEnoughData) {
      return {
        ...
        confidence: 'low',
        message:
          'Insufficient historical data for reliable predictions. Need at least 3 months of transaction history.',
    ...
    for (let i = 1; i <= monthsToPredict; i++) {
      ...
      const monthPrediction = this.predictMonthlySpending(
        historicalAnalysis,
        futureDate,
        options
      );
  ...
  static analyzeHistoricalPatterns(transactions) {
    ...
    const variance =
      monthlySpending.reduce((sum, month) => {
  ...
  static analyzeCategoryPatterns(monthlySpending) {
    ...
    for (const [category, data] of Object.entries(categoryTrends)) {
      if (data.length >= 2) {
        ...
        const secondHalfAvg =
          secondHalf.reduce((sum, amount) => sum + amount, 0) /
          secondHalf.length;
  ...
  static predictMonthlySpending(historicalAnalysis, targetDate, _options = {}) {
    const targetMonth = targetDate.getMonth() + 1;
    const targetYear = targetDate.getFullYear();

    const monthsFromNow = this.calculateMonthsFromNow(targetDate);
    let basePrediction =
      historicalAnalysis.averageMonthlySpending +
    ...
    if (historicalAnalysis.seasonalPatterns.hasPatterns) {
      const seasonalAdjustment = this.getSeasonalAdjustment(
        targetMonth,
        historicalAnalysis.seasonalPatterns
      );
    ...
    const categoryPredictions = this.predictCategoryBreakdown(
      historicalAnalysis,
      basePrediction
    );
    ...
    return {
      ...
      confidence: this.calculateMonthlyConfidence(
        historicalAnalysis,
        monthsFromNow
      ),
      categoryPredictions,
  ...
  static getSeasonalAdjustment(month, seasonalPatterns) {
    ...
    const overallAverage =
      Object.values(seasonalPatterns.seasonalAverages).reduce(
        (sum, avg) => sum + avg,
        0
      ) / Object.keys(seasonalPatterns.seasonalAverages).length;

    const seasonalAverage = seasonalPatterns.seasonalAverages[season];
    return seasonalAverage && overallAverage > 0
      ? seasonalAverage / overallAverage
      : 1.0;
  }
  ...
  static predictCategoryBreakdown(historicalAnalysis, totalPredictedAmount) {
    ...
    for (const [category, pattern] of Object.entries(categoryPatterns)) {
      ...
      predictions.push({
        ...
        confidence:
          pattern.volatility < 0.3
            ? 'high'
            : pattern.volatility < 0.6
              ? 'medium'
              : 'low',
      });
    }
  ...
  static calculateMonthlyConfidence(historicalAnalysis, monthsFromNow) {
    ...
    const lowVolatility =
      ...
      historicalAnalysis.averageMonthlySpending * 0.3;

    const farIntoFuture = monthsFromNow > 6;
    const highVolatility =
      historicalAnalysis.volatility >
      historicalAnalysis.averageMonthlySpending * 0.5;
    ...
    const negativeFactors = [farIntoFuture, highVolatility].filter(
      Boolean
    ).length;
