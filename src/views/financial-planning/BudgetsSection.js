...
export const BudgetsSection = (planningData) => {
    ...
    const render = () => {
        ...
        categoriesWithBudgets.forEach(cat => {
            ...
            if (cat.budget) {
                const utilization = cat.budget.amountLimit > 0 ? (cat.amount / cat.budget.amountLimit) * 100 : 0;
                const diff = cat.budget.amountLimit - cat.amount;
                const diffText = diff >= 0 ? `${formatCurrency(diff)} left` : `${formatCurrency(Math.abs(diff))} over`;
