...
describe('CSS Architecture Foundation', () => {
  ...
  it('should have base files', () => {
    const baseFiles = ['reset.css', 'typography.css', 'index.css'];
  ...
  it('should have CSS custom properties defined in tokens', () => {
    const colorsCss = readFileSync(
      join(stylesDir, 'tokens/colors.css'),
      'utf-8'
    );
    const typographyCss = readFileSync(
      join(stylesDir, 'tokens/typography.css'),
      'utf-8'
    );
    const spacingCss = readFileSync(
      join(stylesDir, 'tokens/spacing.css'),
      'utf-8'
    );
