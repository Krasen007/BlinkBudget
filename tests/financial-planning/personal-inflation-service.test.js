...
describe('PersonalInflationService', () => {
  ...
  beforeEach(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      mockTransactions.push({
        ...
        category: 'Coffee',
        type: 'expense',
        amount: 4.50 + (i * 0.25), // 4.50, 4.75, 5.00, 5.25, 5.50, 5.75
      ...
      mockTransactions.push({
        ...
        category: 'Transport',
        type: 'expense',
        amount: 50 - (i * 1.67), // 50, 48.33, 46.67, 45, 43.33, 41.67
    ...
    mockTransactions.push(
      { id: 'salary-1', category: 'Salary', type: 'income', amount: 3000.00, timestamp: `${currentYear}-${String(currentMonth - 4).padStart(2, '0')}-01T09:00:00Z` },
      { id: 'salary-2', category: 'Salary', type: 'income', amount: 3000.00, timestamp: `${currentYear}-${String(currentMonth - 3).padStart(2, '0')}-01T09:00:00Z` }
    );
  });
  ...
  describe('getMonthlyAverages', () => {
    it('should return monthly averages in correct order', () => {
      const categoryTransactions = mockTransactions.filter(t => t.category === 'Coffee');
      const averages = PersonalInflationService.getMonthlyAverages(categoryTransactions, 6);
      
      expect(averages).toHaveLength(6);
      expect(averages[0]).toBe(5.75); // Most recent (December)
      expect(averages[5]).toBe(4.50); // Oldest (July)
    });
    ...
    it('should filter by date range correctly', () => {
      ...
      const categoryTransactions = mockTransactions.filter(t => t.category === 'Coffee');
      const averages = PersonalInflationService.getMonthlyAverages(categoryTransactions, 3);
  ...
  describe('getTopInflationCategories', () => {
    ...
    it('should filter out zero inflation rates', () => {
      const transactionsWithEmptyCategory = [
        ...mockTransactions,
        { id: '21', category: 'Empty', type: 'expense', amount: 0, timestamp: '2025-12-15T10:00:00Z' }
      ];
  ...
  describe('getMonthlySpendingData', () => {
    it('should return correct monthly spending data', () => {
      ...
      expect(monthlyData[0].amount).toBeCloseTo(4.50, 2); // Oldest
  ...
  describe('validateCategoryData', () => {
    ...
    it('should reject categories with insufficient time range', () => {
      ...
      expect(validation.hasData).toBe(false);
      expect(validation.reason).toBe('Need more than 12 months of data');
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should handle division by zero gracefully', () => {
      const transactionsWithZeroOldPrice = [
        { id: '1', category: 'Test', type: 'expense', amount: 0, timestamp: '2025-07-15T10:00:00Z' },
        { id: '2', category: 'Test', type: 'expense', amount: 10, timestamp: '2025-12-15T10:00:00Z' },
      ];
    ...
    it('should handle NaN values gracefully', () => {
      const transactionsWithNaN = [
        { id: '1', category: 'Test', type: 'expense', amount: NaN, timestamp: '2025-07-15T10:00:00Z' },
        { id: '2', category: 'Test', type: 'expense', amount: 10, timestamp: '2025-12-15T10:00:00Z' },
      ];
    ...
    it('should handle empty transaction arrays', () => {
      const topCategories = PersonalInflationService.getTopInflationCategories([], 3, 6);
